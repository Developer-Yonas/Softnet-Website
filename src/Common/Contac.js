...
const Contac = () => {
  ...
  return (
    <div className={`contact-infoo hidden md:flex-row cursor-pointer mt-0 w-auto justify-between items-center px-4 md:flex sm:hidden bg-white ${isTop ? '' : 'md:hidden sm:hidden'}`}>
      <p className="flex mt-1 mb-0 hover:text-violet-700  text-violet-500 text-base sm:text-sm md:text-lg  md:mr-4 md:mb-0"><FaPhone /> :+251(0)15 57 86 57</p>
      <p className="flex mt-1 mb-0 hover:text-violet-700  text-violet-500 text-base sm:text-sm md:text-lg "><FaMapMarkerAlt />: Mexico , Shewa Bakery Bldg. 4th Floor - Infront of K Kare Center
